...
class EditarPacientePanel extends JPanel {
	...
	public EditarPacientePanel(int index) {
		setSize(540, 450);
		setLayout(null);
		this.index = index;
		
		Paciente paciente = Sistema.getPacientes().get(index);
		...
		fldNome = new JTextField(paciente.getNome());
		...
		fldGenero.setSelectedIndex(paciente.getGenero());
		...
		fldCpf.setText(paciente.getCpf());
		...
		fldAltura = new JTextField(paciente.getAltura() + "");
		...
		fldPeso = new JTextField(paciente.getPeso() + "");
		...
		fldEmail = new JTextField(paciente.getEmail());
		...
		fldTelefone.setText(paciente.getTelefone());
		...
		fldRua = new JTextField(paciente.getEndereco().getRua());
		...
		if(paciente.getEndereco().getNumero() != -1) {
			fldNumero.setText(paciente.getEndereco().getNumero() + "");
		}
		...
		fldBairro = new JTextField(paciente.getEndereco().getBairro());
		...
		fldCidade = new JTextField(paciente.getEndereco().getCidade());
		...
		fldCep.setText(paciente.getEndereco().getCep());
		contato.add(fldCep);
		...
		fldDescricao = new JTextArea(paciente.getDescricao());
...
// Action Listeners
	private final ActionListener ok = new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
			...
			if(!erro) {
				...
				Sistema.getPacientes().set(index, new Paciente(dataNascimento, endereco, cpf, descricao, email, nome, telefone, altura, peso, genero));
	...
	private final ActionListener cancelar = arg0 -> {
		((JFrame) getRootPane().getParent()).dispose(); // Fecha a Janela de Edição
	};
