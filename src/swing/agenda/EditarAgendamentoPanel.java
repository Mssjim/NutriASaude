...
class EditarAgendamentoPanel extends JPanel {
	...
	public EditarAgendamentoPanel(int index) {
		setSize(540, 450);
		setLayout(null);
		this.index = index;
		...
		fldCpf.setText(agendamento.getPaciente().getCpf());
		paciente.add(fldCpf);
		...
		fldDescricao.setText(agendamento.getDescricao());
		...
		fldData.setText(String.format("%02d", agendamento.getData().getDia()) + "/" + String.format("%02d", agendamento.getData().getMes()) + "/" + agendamento.getData().getAno());
		data.add(fldData);
		...
		fldHorario.setText(String.format("%02d", agendamento.getData().getHora()) + ":" + String.format("%02d", agendamento.getData().getMinutos()));
		data.add(fldHorario);
		...
		pesquisar.actionPerformed(null); // Exibe os Dados do Paciente
	}
...
private final ActionListener verificarAgenda = new ActionListener() {
	...
	private final ActionListener ok = new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
			...
			if(!erro) {
				Sistema.getAgendamentos().set(index, new Agendamento(data, paciente, fldDescricao.getText()));
